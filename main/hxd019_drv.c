...
void IRDA_INIT()
{
	...
	IRDA_SDA_H();
	sda_print("IRDA_INIT H");
	IRDA_SET_SDA_OUT();

	IRDA_SCL_H();
	scl_print("IRDA_INIT H");
	IRDA_SET_SCL_OUT();
...
czx_u16 IRDA_start(void)
{
	...
	IRDA_SET_SCL_OUT();
	IRDA_SCL_H();
	scl_print("IRDA_start H");

	IRDA_SDA_H();
	sda_print("IRDA_start H");

	IRDA_delay_10us(IRDA_DELAY_40US);
	IRDA_delay_10us(IRDA_DELAY_40US);
	IRDA_SDA_L();
	sda_print("IRDA_start L");
...
UINT16 I2CReadData(UINT8 *pbData)
{
	...
	while (i--)
	{
		...
		readdata |= IRDA_GET_DATA()?0x01:0x00; //读不到时可以这样试：readdata |= IRDA_GET_DATA()?0x01:0x00;
